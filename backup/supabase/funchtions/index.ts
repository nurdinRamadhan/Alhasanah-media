...
const TOPIC_TASK: Record<TopicKey, string> = {
    LAPORAN:   "Buat ringkasan eksekutif menyeluruh semua aspek pesantren hari ini. Soroti hal yang perlu perhatian segera.",
    KESEHATAN: "Analisa kondisi kesehatan santri. Deteksi potensi wabah jika ada keluhan berulang. Rekomendasikan tindakan.",
    KEUANGAN:  "Ringkas arus kas bulan ini. Tampilkan surplus/defisit, kategori pengeluaran terbesar, dan tagihan belum lunas.",
    DISIPLIN:  "Evaluasi kedisiplinan santri 7 hari terakhir. Tampilkan top pelanggar, jenis pelanggaran dominan, dan santri teladan.",
    TAHFIDZ:   "Ringkas progres hafalan santri. Tampilkan mumtaz, rata-rata, santri butuh bimbingan, dan tren murojaah.",
    IZIN:      "Ringkas data perizinan aktif. Siapa yang sedang izin? Ada yang belum kembali sesuai jadwal?",
    PRESTASI:  "Rekap prestasi santri terbaru. Siapa yang berprestasi? Kategori prestasi apa yang mendominasi?",
...
ATURAN OUTPUT:
① Maksimal 250 kata — Telegram tidak cocok untuk teks panjang
② Format Telegram (parse_mode Markdown v1): *bold*, _italic_, \`code\`
③ Baris 2: *Status:* 🟢 Normal / 🟡 Waspadai / 🔴 Kritis + keterangan singkat  
④ Gunakan bullet • untuk poin data (BUKAN tabel)
⑤ DILARANG: basa-basi pembuka, saran > 1 kalimat
⑥ Akhiri SELALU dengan: ⚡ *Aksi:* [1 kalimat konkret & spesifik]

FORMAT WAJIB:
*[EMOJI] [JUDUL SINGKAT]*
*Status:* [🟢/🟡/🔴] — [keterangan ≤ 5 kata]

• [data spesifik 1]
• [data spesifik 2]  
• [data spesifik 3]

⚡ *Aksi:* [satu tindakan konkret]`;
    ...
    try {
        ...
        const geminiRes = await fetch(GEMINI_URL, {
            ...
            body: JSON.stringify({
                ...
                generationConfig: {
                    temperature:     0.5,   // Lebih deterministik untuk laporan data
                    topK:            40,
                    topP:            0.92,
                    maxOutputTokens: 1024,  // Cukup untuk format Telegram (max 200 kata)
                },
            }),
        });
