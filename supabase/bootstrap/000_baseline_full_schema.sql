...
CREATE OR REPLACE FUNCTION "app_private"."audit_sensitive_access"("p_action" "text", "p_resource" "text", "p_record_id" "text", "p_details" "jsonb" DEFAULT '{}'::"jsonb") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
begin
  insert into public.audit_logs(user_id, user_name, user_role, action, resource, record_id, details, meta_info)
  select auth.uid(), p.full_name, p.role, p_action, p_resource, p_record_id, coalesce(p_details, '{}'::jsonb), 'sensitive-santri-access'
...
ALTER FUNCTION "app_private"."audit_sensitive_access"("p_action" "text", "p_resource" "text", "p_record_id" "text", "p_details" "jsonb") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."current_profile_role"() RETURNS "text"
...
ALTER FUNCTION "app_private"."current_profile_role"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."current_user_has_role"("allowed_roles" "text"[]) RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public', 'pg_temp'
    AS $$
  select exists (
    ...
    where p.id = auth.uid()
      and coalesce(p.is_active, true) = true
      and lower(p.role) = any(allowed_roles)
  );
$$;


ALTER FUNCTION "app_private"."current_user_has_role"("allowed_roles" "text"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."current_user_role"() RETURNS "text"
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
  select lower(role)
...
CREATE OR REPLACE FUNCTION "app_private"."decrypt_text"("p_value" "bytea") RETURNS "text"
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'app_private', 'public', 'extensions'
    AS $$
  select case
    when p_value is null then null
    else extensions.pgp_sym_decrypt(p_value, app_private.require_santri_crypto_key())
  end
$$;


ALTER FUNCTION "app_private"."decrypt_text"("p_value" "bytea") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."emis_agama_code"("p_value" "text") RETURNS "text"
...
ALTER FUNCTION "app_private"."emis_agama_code"("p_value" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."emis_distance_category"("p_value" numeric) RETURNS "text"
    LANGUAGE "sql" IMMUTABLE
    SET "search_path" TO 'public'
    AS $$
  select case when p_value is null then null when p_value < 1 then '1' else '2' end
$$;


ALTER FUNCTION "app_private"."emis_distance_category"("p_value" numeric) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."emis_education_code"("p_value" "text") RETURNS "text"
    LANGUAGE "sql" IMMUTABLE
    SET "search_path" TO 'public'
    AS $_$
  select case upper(trim(coalesce(p_value, '')))
    when 'TIDAK SEKOLAH' then '01'
    when 'PUTUS SD' then '02'
    when 'SD / SEDERAJAT' then '03'
    when 'SD/SEDERAJAT' then '03'
    when 'SMP / SEDERAJAT' then '04'
    when 'SMP/SEDERAJAT' then '04'
    when 'SMA / SEDERAJAT' then '05'
    when 'SMA/SEDERAJAT' then '05'
    when 'D1' then '06'
    when 'D2' then '07'
    when 'D3' then '08'
    ...
    when 'S2' then '10'
    when 'S3' then '11'
    else case when trim(coalesce(p_value, '')) ~ '^(0[1-9]|1[01])$' then trim(p_value) else null end
  end
$_$;


ALTER FUNCTION "app_private"."emis_education_code"("p_value" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."emis_income_code"("p_value" "text") RETURNS "text"
    LANGUAGE "sql" IMMUTABLE
    SET "search_path" TO 'public'
    AS $_$
  select case upper(trim(coalesce(p_value, '')))
    when 'KURANG DARI 500.000' then '1'
    when 'RP 0 - RP 500.000' then '1'
    when '500.000 - 999.999' then '2'
    when '1.000.000 - 1.999.999' then '3'
    when '2.000.000 - 4.999.999' then '4'
    when '5.000.000 - 20.000.000' then '5'
    when 'LEBIH DARI 20.000.000' then '6'
    else case when trim(coalesce(p_value, '')) ~ '^[1-6]$' then trim(p_value) else null end
  end
$_$;


ALTER FUNCTION "app_private"."emis_income_code"("p_value" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."emis_job_code"("p_value" "text") RETURNS "text"
    LANGUAGE "sql" IMMUTABLE
    SET "search_path" TO 'public'
    AS $_$
  select case upper(trim(coalesce(p_value, '')))
    when 'TIDAK BEKERJA' then '01'
    when 'NELAYAN' then '02'
    when 'PETANI' then '03'
    when 'PETERNAK' then '04'
    when 'PNS / TNI / POLRI' then '05'
    when 'PNS/TNI/POLRI' then '05'
    when 'PNS' then '05'
    when 'TNI' then '05'
    when 'POLRI' then '05'
    when 'KARYAWAN SWASTA' then '06'
    when 'SWASTA' then '06'
    when 'PEDAGANG KECIL' then '07'
    when 'PEDAGANG BESAR' then '08'
    when 'WIRASWASTA' then '09'
    when 'WIRAUSAHA' then '10'
    when 'BURUH' then '11'
    when 'PENSIUNAN' then '12'
    when 'LAINNYA' then '99'
    when 'LAIN-LAIN' then '99'
    else case when trim(coalesce(p_value, '')) ~ '^(0[1-9]|1[0-2]|99)$' then trim(p_value) else null end
  end
$_$;


ALTER FUNCTION "app_private"."emis_job_code"("p_value" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."emis_parent_status_code"("p_value" "text") RETURNS "text"
    LANGUAGE "sql" IMMUTABLE
    SET "search_path" TO 'public'
    AS $$
  select case upper(trim(coalesce(p_value, '')))
    when 'HIDUP' then 'HIDUP'
    when 'MASIH_HIDUP' then 'HIDUP'
    when 'MASIH HIDUP' then 'HIDUP'
    when 'MENINGGAL' then 'MENINGGAL'
    when 'SUDAH MENINGGAL' then 'MENINGGAL'
    when 'TANPA KETERANGAN' then 'TANPA KETERANGAN'
    when 'TIDAK_DIKETAHUI' then 'TANPA KETERANGAN'
    when 'TIDAK DIKETAHUI' then 'TANPA KETERANGAN'
    else nullif(trim(coalesce(p_value, '')), '')
  end
$$;


ALTER FUNCTION "app_private"."emis_parent_status_code"("p_value" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."emis_program_pesantren_code"("p_value" "text") RETURNS "text"
    LANGUAGE "sql" IMMUTABLE
    SET "search_path" TO 'public'
    AS $_$
  select case upper(trim(coalesce(p_value, '')))
    when 'TAHFIDZ' then '05'
    when 'TAHFIDZ AL-QURAN' then '05'
    when 'TAHFIDZ AL-QUR''AN' then '05'
    when 'KITAB' then '06'
    when 'KAJIAN KITAB KUNING' then '06'
    else case when trim(coalesce(p_value, '')) ~ '^\d{2}$' then trim(p_value) else null end
  end
$_$;


ALTER FUNCTION "app_private"."emis_program_pesantren_code"("p_value" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."emis_status_mukim_code"("p_value" "text") RETURNS "text"
    LANGUAGE "sql" IMMUTABLE
    SET "search_path" TO 'public'
    AS $_$
  select case upper(trim(coalesce(p_value, '')))
    when 'MUKIM' then '4'
    when 'TIDAK_MUKIM' then '1'
    when 'TIDAK MUKIM' then '1'
    when 'KALONG' then '3'
    when 'ASRAMA PESANTREN' then '4'
    else case when trim(coalesce(p_value, '')) ~ '^[1-9]$' then trim(p_value) else null end
  end
$_$;


ALTER FUNCTION "app_private"."emis_status_mukim_code"("p_value" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."encrypt_text"("p_value" "text") RETURNS "bytea"
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'app_private', 'public', 'extensions'
    AS $$
  select case
    when p_value is null or btrim(p_value) = '' then null
    else extensions.pgp_sym_encrypt(p_value, app_private.require_santri_crypto_key(), 'cipher-algo=aes256')
  end
$$;


ALTER FUNCTION "app_private"."encrypt_text"("p_value" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."enforce_wallet_account_mutation"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    SET "search_path" TO ''
    AS $$
begin
  if current_setting('app.wallet_mutation', true) is distinct from 'on' then
    raise exception 'Direct wallet balance mutation is not allowed';
  end if;
  return new;
end;
$$;
...
ALTER FUNCTION "app_private"."enforce_wallet_account_mutation"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."enforce_wallet_ledger_append_only"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    SET "search_path" TO ''
    AS $$
begin
...
ALTER FUNCTION "app_private"."enforce_wallet_ledger_append_only"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."enqueue_chat_message_notification"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
declare
...
ALTER FUNCTION "app_private"."enqueue_chat_message_notification"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."enqueue_forum_comment_notification"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
declare
...
begin
  ...
  if v_thread_author is null or v_thread_author = new.author_id then
    return new;
  end if;
  ...
  if not exists (
    ...
    where a.id = v_thread_author
      and coalesce(a.forum_notify_replies, true) = true
  ) then
    return new;
  end if;

  select coalesce(a.full_name, p.full_name, 'Alumni Al-Hasanah')
  ...
  values (
    v_thread_author,
    'Komentar baru di Forum Alumni',
    coalesce(v_commenter_name, 'Alumni Al-Hasanah') || ' membalas postingan Anda.',
    jsonb_build_object(
      'type', 'forum_comment',
...
ALTER FUNCTION "app_private"."enqueue_forum_comment_notification"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."enqueue_forum_reaction_notification"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
declare
  ...
  v_thread_id uuid;
  v_actor_name text;
  v_target_pref boolean;
begin
  if new.thread_id is not null then
    select author_id, id into v_target_user, v_thread_id
    from public.forum_threads
    where id = new.thread_id
      and status = 'published'
      and deleted_at is null;
  elsif new.comment_id is not null then
    select c.author_id, c.thread_id into v_target_user, v_thread_id
    from public.forum_comments c
    join public.forum_threads t on t.id = c.thread_id
  ...
  select coalesce(a.forum_notify_reactions, true)
    into v_target_pref
  from public.alumni_data a
  where a.id = v_target_user;

  if coalesce(v_target_pref, true) = false then
    return new;
  end if;
  ...
  values (
    v_target_user,
    'Reaksi baru di Forum Alumni',
    coalesce(v_actor_name, 'Alumni Al-Hasanah') || ' menyukai konten Anda.',
    jsonb_build_object(
      'type', 'forum_reaction',
...
ALTER FUNCTION "app_private"."enqueue_forum_reaction_notification"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."enqueue_forum_report_notification"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
begin
  insert into public.notification_queue (user_id, title, body, data, source_table)
  select p.id,
         'Laporan konten forum baru',
         'Ada laporan forum yang perlu ditinjau admin.',
         jsonb_build_object(
           'type', 'forum_report',
           'report_id', new.id,
           'thread_id', new.thread_id,
         ...
         'forum_reports'
  from public.profiles p
  where lower(coalesce(p.role, '')) in ('super_admin', 'kesantrian', 'rois')
...
ALTER FUNCTION "app_private"."enqueue_forum_report_notification"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."forum_comment_counter"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
begin
...
ALTER FUNCTION "app_private"."forum_comment_counter"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."forum_reaction_counter"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
begin
  if tg_op = 'INSERT' then
    if new.thread_id is not null then
      update public.forum_threads set reaction_count = reaction_count + 1 where id = new.thread_id;
    else
      update public.forum_comments set reaction_count = reaction_count + 1 where id = new.comment_id;
    end if;
    return new;
  elsif tg_op = 'DELETE' then
    if old.thread_id is not null then
      update public.forum_threads set reaction_count = greatest(reaction_count - 1, 0) where id = old.thread_id;
    else
      update public.forum_comments set reaction_count = greatest(reaction_count - 1, 0) where id = old.comment_id;
    end if;
    return old;
  end if;
  return null;
...
ALTER FUNCTION "app_private"."forum_reaction_counter"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."has_recent_chat_message"("p_user_id" "uuid") RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
  select exists (
    ...
    where cm.sender_id = p_user_id
      and cm.created_at > now() - interval '2 seconds'
  );
$$;


ALTER FUNCTION "app_private"."has_recent_chat_message"("p_user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."hash_identifier"("p_value" "text") RETURNS "text"
    LANGUAGE "sql" IMMUTABLE
    SET "search_path" TO 'extensions', 'public'
    AS $$
  select case
    when p_value is null or btrim(p_value) = '' then null
    else encode(extensions.digest(lower(regexp_replace(btrim(p_value), '\\s+', '', 'g')), 'sha256'), 'hex')
  end
$$;


ALTER FUNCTION "app_private"."hash_identifier"("p_value" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."is_active_alumni"("p_user_id" "uuid") RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
  select exists (
    select 1
    from public.profiles p
    join public.alumni_data a on a.id = p.id
    where p.id = p_user_id
      and p.role = 'alumni'
      and p.is_active = true
  )
  or app_private.is_current_user_forum_admin();
$$;


ALTER FUNCTION "app_private"."is_active_alumni"("p_user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."is_chat_blocked"("p_user_a" "uuid", "p_user_b" "uuid") RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
  select exists (
    select 1 from public.chat_blocks cb
    where (cb.blocker_id = p_user_a and cb.blocked_id = p_user_b)
       or (cb.blocker_id = p_user_b and cb.blocked_id = p_user_a)
  );
$$;


ALTER FUNCTION "app_private"."is_chat_blocked"("p_user_a" "uuid", "p_user_b" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."is_chat_participant"("p_conversation_id" "uuid", "p_user_id" "uuid" DEFAULT "auth"."uid"()) RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
  select exists (
...
ALTER FUNCTION "app_private"."is_chat_participant"("p_conversation_id" "uuid", "p_user_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."is_current_user_active_alumni"() RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
  select exists (
    ...
    from public.profiles
    where id = (select auth.uid())
      and lower(coalesce(role, '')) = 'alumni'
      and coalesce(is_active, false) = true
  );
$$;


ALTER FUNCTION "app_private"."is_current_user_active_alumni"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."is_current_user_forum_admin"() RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
  select exists (
    ...
    from public.profiles
    where id = (select auth.uid())
      and lower(coalesce(role, '')) in ('super_admin', 'kesantrian', 'rois')
      and coalesce(is_active, false) = true
  );
$$;


ALTER FUNCTION "app_private"."is_current_user_forum_admin"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."is_forum_member_or_admin"() RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
  select (select app_private.is_current_user_active_alumni())
      or (select app_private.is_current_user_forum_admin());
$$;


ALTER FUNCTION "app_private"."is_forum_member_or_admin"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."is_santri_admin"() RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public', 'app_private'
    AS $$
  select app_private.current_user_role() = any(array['super_admin','admin_kesantrian','admin_bendahara','kesantrian','bendahara','rois','dewan'])
$$;


ALTER FUNCTION "app_private"."is_santri_admin"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."is_wallet_admin"() RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
  select coalesce(app_private.current_user_role() in ('super_admin','bendahara'), false)
$$;


ALTER FUNCTION "app_private"."is_wallet_admin"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."is_wallet_auditor"() RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
  select coalesce(app_private.current_user_role() in ('super_admin','bendahara','rois','dewan'), false)
$$;


ALTER FUNCTION "app_private"."is_wallet_auditor"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."mark_forum_report_target_pending_review"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
begin
...
ALTER FUNCTION "app_private"."mark_forum_report_target_pending_review"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."mask_text"("p_value" "text", "p_visible" integer DEFAULT 2) RETURNS "text"
    LANGUAGE "sql" IMMUTABLE
    SET "search_path" TO 'pg_catalog'
    AS $$
  select case
    when p_value is null or p_value = '' then null
    when length(p_value) <= greatest(p_visible, 1) then repeat('*', length(p_value))
    else left(p_value, greatest(p_visible, 1)) || repeat('*', greatest(length(p_value) - greatest(p_visible, 1), 0))
  end
$$;


ALTER FUNCTION "app_private"."mask_text"("p_value" "text", "p_visible" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."refresh_rag_document_chunk_count"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'pg_temp'
    AS $$
begin
  if tg_op = 'INSERT' then
    ...
    update public.rag_documents set chunk_count = greatest(chunk_count - 1, 0), updated_at = now() where id = old.document_id;
    update public.rag_documents set chunk_count = chunk_count + 1, updated_at = now() where id = new.document_id;
    return new;
  end if;
  return new;
end;
...
ALTER FUNCTION "app_private"."refresh_rag_document_chunk_count"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."require_santri_crypto_key"() RETURNS "text"
    LANGUAGE "plpgsql" STABLE SECURITY DEFINER
    SET "search_path" TO 'app_private', 'vault', 'public'
    AS $$
declare
  v_key text;
begin
  v_key := app_private.santri_crypto_key();
  if v_key is null or length(v_key) < 32 then
    raise exception 'Santri encryption key is not configured';
  end if;
  return v_key;
end;
$$;
...
ALTER FUNCTION "app_private"."require_santri_crypto_key"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."sanitize_chat_message_report"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'app_private'
    AS $$
begin
...
ALTER FUNCTION "app_private"."sanitize_chat_message_report"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."sanitize_chat_notification_queue"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'app_private'
    AS $$
begin
...
ALTER FUNCTION "app_private"."sanitize_chat_notification_queue"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."santri_crypto_key"() RETURNS "text"
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'vault', 'public'
    AS $$
  select decrypted_secret
  from vault.decrypted_secrets
  where name = 'santri_field_encryption_key_v1'
  limit 1
$$;


ALTER FUNCTION "app_private"."santri_crypto_key"() OWNER TO "postgres";
...
ALTER FUNCTION "app_private"."set_updated_at"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."touch_chat_conversation_from_message"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
begin
...
ALTER FUNCTION "app_private"."touch_chat_conversation_from_message"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."touch_chat_conversation_updated_at"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
begin
...
ALTER FUNCTION "app_private"."touch_chat_conversation_updated_at"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."touch_chat_presence_updated_at"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
begin
...
ALTER FUNCTION "app_private"."touch_chat_presence_updated_at"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "app_private"."update_alumni_admin_profile"("p_alumni_id" "uuid", "p_full_name" "text" DEFAULT NULL::"text", "p_is_active" boolean DEFAULT NULL::boolean) RETURNS "public"."profiles"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
declare
...
begin
  if not app_private.is_current_user_forum_admin() then
    raise exception 'Akses ditolak. Hanya admin forum yang dapat mengelola profil alumni.' using errcode = '42501';
  end if;

  if not exists (select 1 from public.alumni_data where id = p_alumni_id) then
    raise exception 'Profil alumni tidak ditemukan.' using errcode = 'P0002';
  end if;
...
ALTER FUNCTION "app_private"."update_alumni_admin_profile"("p_alumni_id" "uuid", "p_full_name" "text", "p_is_active" boolean) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."wallet_ledger_hash"("p_santri_nis" "text", "p_direction" "text", "p_category" "text", "p_amount" bigint, "p_balance_before" bigint, "p_balance_after" bigint, "p_idempotency_key" "text", "p_prev_hash" "text", "p_nonce" "text", "p_signature" "text", "p_created_at" timestamp with time zone) RETURNS "text"
    LANGUAGE "sql" IMMUTABLE
...
ALTER FUNCTION "app_private"."wallet_ledger_hash"("p_santri_nis" "text", "p_direction" "text", "p_category" "text", "p_amount" bigint, "p_balance_before" bigint, "p_balance_after" bigint, "p_idempotency_key" "text", "p_prev_hash" "text", "p_nonce" "text", "p_signature" "text", "p_created_at" timestamp with time zone) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "app_private"."wallet_touch_updated_at"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public'
    AS $$
begin
...
ALTER FUNCTION "app_private"."wallet_touch_updated_at"() OWNER TO "postgres";
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."alumni_follows";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."chat_blocks";
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."chat_message_reports";
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."chat_user_presence";
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."forum_reactions";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."forum_reports";
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."transaksi_keuangan";
...
REVOKE ALL ON FUNCTION "app_private"."audit_sensitive_access"("p_action" "text", "p_resource" "text", "p_record_id" "text", "p_details" "jsonb") FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."audit_sensitive_access"("p_action" "text", "p_resource" "text", "p_record_id" "text", "p_details" "jsonb") TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."current_profile_role"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."current_profile_role"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."current_user_has_role"("allowed_roles" "text"[]) FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."current_user_has_role"("allowed_roles" "text"[]) TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."current_user_role"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."current_user_role"() TO "service_role";
GRANT ALL ON FUNCTION "app_private"."current_user_role"() TO "authenticated";



REVOKE ALL ON FUNCTION "app_private"."decrypt_text"("p_value" "bytea") FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."decrypt_text"("p_value" "bytea") TO "service_role";



GRANT ALL ON FUNCTION "app_private"."emis_agama_code"("p_value" "text") TO "service_role";



GRANT ALL ON FUNCTION "app_private"."emis_distance_category"("p_value" numeric) TO "service_role";



GRANT ALL ON FUNCTION "app_private"."emis_education_code"("p_value" "text") TO "service_role";



GRANT ALL ON FUNCTION "app_private"."emis_income_code"("p_value" "text") TO "service_role";



GRANT ALL ON FUNCTION "app_private"."emis_job_code"("p_value" "text") TO "service_role";



GRANT ALL ON FUNCTION "app_private"."emis_parent_status_code"("p_value" "text") TO "service_role";



GRANT ALL ON FUNCTION "app_private"."emis_program_pesantren_code"("p_value" "text") TO "service_role";



GRANT ALL ON FUNCTION "app_private"."emis_status_mukim_code"("p_value" "text") TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."encrypt_text"("p_value" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."encrypt_text"("p_value" "text") TO "service_role";



GRANT ALL ON FUNCTION "app_private"."enforce_wallet_account_mutation"() TO "service_role";



GRANT ALL ON FUNCTION "app_private"."enforce_wallet_ledger_append_only"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."enqueue_chat_message_notification"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."enqueue_chat_message_notification"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."enqueue_forum_comment_notification"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."enqueue_forum_comment_notification"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."enqueue_forum_reaction_notification"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."enqueue_forum_reaction_notification"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."enqueue_forum_report_notification"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."enqueue_forum_report_notification"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."forum_comment_counter"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."forum_comment_counter"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."forum_reaction_counter"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."forum_reaction_counter"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."has_recent_chat_message"("p_user_id" "uuid") FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."has_recent_chat_message"("p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "app_private"."has_recent_chat_message"("p_user_id" "uuid") TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."hash_identifier"("p_value" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."hash_identifier"("p_value" "text") TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."is_active_alumni"("p_user_id" "uuid") FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."is_active_alumni"("p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "app_private"."is_active_alumni"("p_user_id" "uuid") TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."is_chat_blocked"("p_user_a" "uuid", "p_user_b" "uuid") FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."is_chat_blocked"("p_user_a" "uuid", "p_user_b" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "app_private"."is_chat_blocked"("p_user_a" "uuid", "p_user_b" "uuid") TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."is_chat_participant"("p_conversation_id" "uuid", "p_user_id" "uuid") FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."is_chat_participant"("p_conversation_id" "uuid", "p_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "app_private"."is_chat_participant"("p_conversation_id" "uuid", "p_user_id" "uuid") TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."is_current_user_active_alumni"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."is_current_user_active_alumni"() TO "authenticated";
GRANT ALL ON FUNCTION "app_private"."is_current_user_active_alumni"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."is_current_user_forum_admin"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."is_current_user_forum_admin"() TO "authenticated";
GRANT ALL ON FUNCTION "app_private"."is_current_user_forum_admin"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."is_forum_member_or_admin"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."is_forum_member_or_admin"() TO "authenticated";
GRANT ALL ON FUNCTION "app_private"."is_forum_member_or_admin"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."is_santri_admin"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."is_santri_admin"() TO "service_role";



GRANT ALL ON FUNCTION "app_private"."is_wallet_admin"() TO "service_role";



GRANT ALL ON FUNCTION "app_private"."is_wallet_auditor"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."mark_forum_report_target_pending_review"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."mark_forum_report_target_pending_review"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."mask_text"("p_value" "text", "p_visible" integer) FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."mask_text"("p_value" "text", "p_visible" integer) TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."refresh_rag_document_chunk_count"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."refresh_rag_document_chunk_count"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."require_santri_crypto_key"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."require_santri_crypto_key"() TO "service_role";



GRANT ALL ON FUNCTION "app_private"."sanitize_chat_message_report"() TO "service_role";



GRANT ALL ON FUNCTION "app_private"."sanitize_chat_notification_queue"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."santri_crypto_key"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."santri_crypto_key"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."set_updated_at"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."set_updated_at"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."touch_chat_conversation_from_message"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."touch_chat_conversation_from_message"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."touch_chat_conversation_updated_at"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."touch_chat_conversation_updated_at"() TO "service_role";



REVOKE ALL ON FUNCTION "app_private"."touch_chat_presence_updated_at"() FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."touch_chat_presence_updated_at"() TO "service_role";
...
REVOKE ALL ON FUNCTION "app_private"."update_alumni_admin_profile"("p_alumni_id" "uuid", "p_full_name" "text", "p_is_active" boolean) FROM PUBLIC;
GRANT ALL ON FUNCTION "app_private"."update_alumni_admin_profile"("p_alumni_id" "uuid", "p_full_name" "text", "p_is_active" boolean) TO "service_role";



GRANT ALL ON FUNCTION "app_private"."wallet_ledger_hash"("p_santri_nis" "text", "p_direction" "text", "p_category" "text", "p_amount" bigint, "p_balance_before" bigint, "p_balance_after" bigint, "p_idempotency_key" "text", "p_prev_hash" "text", "p_nonce" "text", "p_signature" "text", "p_created_at" timestamp with time zone) TO "service_role";



GRANT ALL ON FUNCTION "app_private"."wallet_touch_updated_at"() TO "service_role";
